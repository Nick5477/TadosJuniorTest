namespace Infrastructure.TypedFactory
{
    using System;
    ...
    public static class TypedFactoryExtensions
    {
        private static readonly AssemblyBuilder AssemblyBuilder = AssemblyBuilder
            ...
            new AssemblyName("TypedFactoryAssembly"), 
        ...
        private static readonly ConstructorInfo TypedFactoryBaseConstructorInfo = TypedFactoryBaseType
            .GetMatchingConstructor(new[] { typeof(IComponentContext) });
            ...
            where TFactoryService : class
        {
            ...
            foreach (MethodInfo methodInfo in factoryServiceType.GetMethods())
            {
                ...
                if (returnType.GenericTypeArguments.Length > 0)
                {
                    ...
                    GenericTypeParameterBuilder[] genericArguments = newMethodInfo
